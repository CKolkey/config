...
local function current_non_main_co()
  ...
  if select("#", table.unpack(data)) == 2 then
    local co, is_main = table.unpack(data)
    if is_main then
      return nil
    end
    return co
...
function nio.tasks.run(func, cb)
  ...
  local future = require("lib.nio").control.future()
  ...
  local function close_task(result, err)
    ...
    if err then
      ...
      future.set(table.unpack(result))
      if cb then
        cb(true, table.unpack(result))
      end
    end
  end
  ...
  local function step(...)
    ...
    if coroutine.status(co) == "dead" then
      print(hs.inspect(table.unpack(yielded, 2, #yielded)))
      close_task({ table.unpack(yielded, 2, #yielded) })
      return
    end

    local _, nargs, err_or_fn = table.unpack(yielded)
    ...
    local args = { select(4, table.unpack(yielded)) }

    args[nargs] = step

    err_or_fn(table.unpack(args, 1, nargs))
  end
